...
// 페르소나 전면 몰입(2026-07-23): 궁합은 연리(맞닿은 가지를 돌보는 이)의 목소리 — 그윽하고
// 느긋한 요체, 나무·바람·그늘의 비유, 두 사람을 같은 온도로(한쪽 편들지 않기).

const RELATION_TEXT: Record<DailyRelation, string> = {
  동행:
    "두 분은 같은 오행의 결을 지녔어요. 같은 흙에서 자란 두 나무처럼, 말하지 않아도 통하는 순간이 많지요. 다만 닮은 만큼 그늘도 닮아 있다는 것 — 그것만 서로 기억해 두면 돼요.",
  채움:
    "상대의 기운이 당신 쪽으로 물을 대주는 조합이에요. 함께 있으면 어쩐지 마음이 차오르고, 배우게 되는 것이 많지요. 받은 온기를 한 번씩 되돌려주면 이 가지는 더 오래 푸르러요.",
  발산:
    "당신의 기운이 상대를 자라게 하는 조합이에요. 당신이 드리운 그늘 아래서 상대가 조금씩 피어나지요. 주는 기쁨이 크지만, 가끔은 당신의 뿌리도 함께 돌봐주기로 해요.",
  결실:
    "당신이 이끌고 상대가 다듬어지는, 열매를 함께 맺는 조합이에요. 무언가를 같이 도모할 때 유난히 손발이 맞지요. 이끄는 쪽일수록 상대의 속도를 한 번 더 물어봐주면 좋아요.",
  단련:
    "상대의 기운이 당신을 단단하게 벼려주는 조합이에요. 바람을 맞으며 자란 나무가 뿌리를 깊이 내리듯, 이 관계를 지나며 당신은 분명 자라나지요. 부딪힘이 있는 날엔 한 템포 쉬어가요.",
};

const HARMONY_TEXT: Record<ZodiacHarmony, string> = {
  닮음:
    "별자리의 원소도 같은 결이라, 세상을 바라보는 온도가 비슷해요. 함께 있으면 설명이 줄고 공감이 늘지요.",
  어울림:
    "별자리의 원소가 서로를 살리는 짝이에요. 한쪽이 볕이라면 다른 쪽은 바람 — 함께일 때 각자보다 커지는 조합이지요.",
  다름:
    "별자리의 원소가 서로 다른 결이에요. 뿌리가 다른 두 나무처럼 처음엔 낯설 수 있지만, 그만큼 서로가 못 보던 풍경을 보여줄 수 있는 사이지요.",
};
...
export function bondText(bond: MatchBond | null): string {
  ...
  const parts: string[] = [];
  if (bond.stemCombine)
    parts.push("두 분의 일간은 서로를 끌어안는 합(合)의 짝이에요. 처음 만나도 오래 안 사이처럼 마음이 기우는, 가지가 먼저 알아보는 인연이지요.");
  if (bond.branchBond === "육합")
    parts.push("두 분의 일지도 손을 맞잡는 육합의 자리라, 함께 있는 시간이 자연스럽게 편안해지지요.");
  else if (bond.branchBond === "충")
    parts.push("두 분의 일지는 서로 마주 보는 충(沖)의 자리이기도 해요. 부딪히는 날도 있겠지만, 바람이 나무를 단단하게 하듯 서로를 자라게 하는 인연이지요.");
  return parts.join(" ");
}

const MODE_CLOSING: Record<MatchMode, string> = {
  연인:
    "사랑은 닮음보다 리듬이에요. 서로의 속도를 한 번씩 물어봐주는 것 — 그게 두 가지가 오래 맞닿아 자라는 길이에요.",
  친구:
    "좋은 우정은 자주가 아니라 깊이로 자라요. 문득 생각날 때 건네는 안부 한마디면, 이 결은 오래오래 이어지지요.",
  동료:
    "함께 일할 땐 서로의 강점에 기대보아요. 역할의 결을 나누고 나면, 이 조합은 생각보다 큰 일을 해내지요.",
};
...
/** 오행 상호 보완(심층 궁합의 보상) 문구 — 연리의 결(맞닿아 서로를 채우는 가지). */
export function complementText(c: DeepMatchContext["complement"]): string {
  ...
  if (i.length && p.length)
    return `서로가 서로의 빈 자리를 채워주는, 흔치 않은 조합이에요. 당신은 상대에게 ${list(i)}의 기운을, 상대는 당신에게 ${list(p)}의 기운을 건네주지요. 맞닿은 가지가 서로의 결을 이어가듯, 함께 있는 것만으로 각자의 팔자가 조금 더 온전해져요.`;
  if (i.length)
    return `당신에게는 상대에게 없는 ${list(i)}의 기운이 있어요. 당신의 존재가 상대의 빈 자리를 살며시 메워주는 셈이지요. 곁에 있어주는 것만으로 큰 그늘이 되어줄 거예요.`;
  if (p.length)
    return `상대에게는 당신에게 없는 ${list(p)}의 기운이 있어요. 그 곁에 있으면 당신의 빈 자리가 조용히 채워지지요. 그 온기를 편히 받아들여도 괜찮아요.`;
  return "두 분 모두 각자의 결이 이미 뚜렷한 편이에요. 채움보다는 나란히 서서 같은 곳을 바라보는, 두 그루의 나무 같은 조합이지요.";
}
