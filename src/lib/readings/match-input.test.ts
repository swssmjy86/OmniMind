...
const valid = {
  birthDate: "1992-03-10", birthTime: "14:20", timeUnknown: false, mode: "lover",
};

describe("궁합 심층 상대 입력 검증 (3단계 스펙 §5)", () => {
  it("정상 입력 — 모드 슬러그가 한글 모드로 변환된다", () => {
    expect(parseMatchDeepInput(valid)).toEqual({
      birthDate: "1992-03-10", birthTime: "14:20", timeUnknown: false, mode: "연인",
    });
  });
